...
/* 
  ...
  These webviews block inline video playback heavily.
*/
...
const HeroSection = () => {
  ...
  // For TikTok: true shows the GIF, false shows the real video
  const [useGifFallback, setUseGifFallback] = useState(IS_RESTRICTIVE_APP);
  ...
  const handleClick = useCallback(() => {
    // If we're showing the GIF, hitting the video area means "I want to watch the real video with sound"
    if (useGifFallback) {
      setUseGifFallback(false);
      setIsMuted(false);
      
      // We must wait a tiny bit for React to render the <video> tag, then play it.
      setTimeout(() => {
          ...
          <div className="fade-item order-2 w-full max-w-xl mx-auto lg:max-w-none">
            ...
            >
              <div className="relative w-full bg-[#0a0505]" style={{ paddingBottom: "56.25%" }}>
                ...
                {useGifFallback ? (
                  /* ─── TikTok/IG Fallback: Animated GIF. Cannot trigger native video player ─── */
                  <img 
                  ...
                  /* ─── Normal Web or after user clicks GIF ─── */
                  <video
                    ref={videoRef}
                    src={VIDEO_URL}
                    autoPlay
                ...
                {/* Unmute / Play Overlay */}
                {((useGifFallback) || (isReady && (isMuted || !isPlaying))) && (
                  <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      {useGifFallback && (
                         <span className="font-editorial italic text-foreground/80 text-[11px] tracking-wider bg-black/50 backdrop-blur-sm px-3 py-1 rounded-full mt-2">
                           Toque para ouvir
                         </span>
                      )}
                    </div>
                ...
                {/* Loading (only for real video) */}
                {!useGifFallback && !isReady && (
